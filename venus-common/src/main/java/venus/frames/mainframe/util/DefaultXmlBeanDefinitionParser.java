...
public class DefaultXmlBeanDefinitionParser {
	...
	public static boolean CLASS_CAN_NO_FOUND = false;
	...
	protected void registerBeanDefinition(Element ele) {
		...
		if( !CLASS_CAN_NO_FOUND ){
			
			bdHolder = parseBeanDefinition(ele);
			...
			try{
				...
				logger.error("Error in Registering bean definition ",e);
				return;
			
			}

		}
		//upon update by wj 20050902 for CLASS_CAN_NO_FOUND when init beanfactory

		
		
		logger.debug("Registering bean definition with id '" + bdHolder.getBeanName() + "'");
		this.beanDefinitionReader.getRegistry().registerBeanDefinition(
				bdHolder.getBeanName(), bdHolder.getBeanDefinition());
		if (bdHolder.getAliases() != null) {
			for (int i = 0; i < bdHolder.getAliases().length; i++) {
				this.beanDefinitionReader.getRegistry().registerAlias(
	...
	protected BeanDefinitionHolder parseBeanDefinition(Element ele) {
		...
		if (!StringUtils.hasLength(id)) {
			if (beanDefinition instanceof RootBeanDefinition) {
				...
				id = className;
				int counter = 1;
				while (this.beanDefinitionReader.getRegistry().containsBeanDefinition(id)) {
					counter++;
				...
				throw new BeanDefinitionStoreException(this.resource.getDescription(), "", "Child bean definition has neither 'id' nor 'name'");
			}
		}
...
//			MutablePropertyValues pvs = getPropertyValueSubElements(beanName, ele);
			...
			AbstractBeanDefinition bd = BeanDefinitionReaderUtils.createBeanDefinition(className, parent, this.beanDefinitionReader.getBeanClassLoader());
			...
			if (ele.hasAttribute(SINGLETON_ATTRIBUTE)) {
				//spring4.2.5.RELEASE 没有setSingleton方法
//				bd.setSingleton(TRUE_VALUE.equals(ele.getAttribute(SINGLETON_ATTRIBUTE)));
			...
			throw new BeanDefinitionStoreException(this.resource.getDescription(), beanName, "Bean class [" + className + "] not found", ex);
		}
		catch (NoClassDefFoundError err) {
			throw new BeanDefinitionStoreException(this.resource.getDescription(), beanName, "Class that bean class [" + className + "] depends on not found", err);
		}
	}
	...
	protected void parseConstructorArgElement(String beanName, ConstructorArgumentValues cargs, Element ele)
		...
		if (StringUtils.hasLength(indexAttr)) {
			try {
				...
				if (index < 0) {
					throw new BeanDefinitionStoreException(this.resource.getDescription(), beanName, "'index' cannot be lower than 0");
				}
				...
				throw new BeanDefinitionStoreException(this.resource.getDescription(), beanName, "Attribute 'index' of tag 'constructor-arg' must be an integer");
			}
		}
		else {
			if (StringUtils.hasLength(typeAttr)) {
	...
	protected void parsePropertyElement(String beanName, MutablePropertyValues pvs, Element ele)
		...
		if (!StringUtils.hasLength(propertyName)) {
			throw new BeanDefinitionStoreException(this.resource.getDescription(), beanName, "Tag 'property' must have a 'name' attribute");
		}
	...
	protected Object getPropertyValue(Element ele, String beanName, String propertyName) {
		...
		if (valueRefOrCollectionElement == null) {
			...
			throw new BeanDefinitionStoreException(this.resource.getDescription(), beanName, elementName + " must have a subelement like <value> or <ref>");
		}
	...
	protected Object parsePropertySubelement(Element ele, String beanName) {
		if (ele.getTagName().equals(BEAN_ELEMENT)) {
			...
			if (!StringUtils.hasLength(beanRef)) {
				...
				if (!StringUtils.hasLength(beanRef)) {
					...
					if (!StringUtils.hasLength(beanRef)) {
						throw new BeanDefinitionStoreException(this.resource.getDescription(), beanName, "'bean', 'local' or 'parent' is required for a reference");
					}
			...
			if (!StringUtils.hasLength(beanRef)) {
				...
				beanRef = ele.getAttribute(LOCAL_REF_ATTRIBUTE);
				if (!StringUtils.hasLength(beanRef)) {
					throw new BeanDefinitionStoreException(this.resource.getDescription(), beanName, "Either 'bean' or 'local' is required for an idref");
				}
			}
			return beanRef;
		}
		...
		throw new BeanDefinitionStoreException(this.resource.getDescription(), beanName, "Unknown subelement of <property>: <" + ele.getTagName() + ">");
	}
	...
	protected String getTextValue(Element ele, String beanName) {
		...
		for (int i = 0; i < nl.getLength(); i++) {
			...
			if (item instanceof CharacterData) {
				...
				throw new BeanDefinitionStoreException(this.resource.getDescription(), beanName, "<value> element is just allowed to have text and comment nodes, not: " + item.getClass().getName());
			}
		}
	...
	private static abstract class ManagedLinkedMapCreator {
		...
		private static Map createManagedLinkedMap(int capacity) {
			return new ManagedMap(capacity);
		}
	}
