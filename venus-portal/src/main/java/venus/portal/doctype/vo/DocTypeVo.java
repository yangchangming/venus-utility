...
public class DocTypeVo extends BaseBusinessObject implements Comparable {
    ...
    private TreeSet<DocTypeVo> parent; //栏目的父栏目(多对多)
    private TreeSet<DocTypeVo> children; //栏目的子栏目(多对多)
    private String templateId;//模板ID，此属性字段只做数据存储在持久化时采用 template 与模板表相关联
    private EwpTemplate template;// 栏目的模板
    private String docTemplateId;
    private EwpTemplate docTemplate;
    ...
    public DocTypeVo(String id, String parentID) {
        super.setId(id);
    ...
    public TreeSet<DocTypeVo> getChildren() {
        return children;
    }

    public TreeSet<DocTypeVo> getValidChildren(){
        TreeSet<DocTypeVo> validChildren = newTreeSet(filter(children, new Predicate<DocTypeVo>() {
            public boolean apply(DocTypeVo vo) {
                return vo.getIsValid().equals(IApiConstants.VIEW_DOCTYPE_IS_VALID);
            }
        }));
    ...
    public TreeSet<DocTypeVo> getParent() {
        return parent;
    }

    public void setParent(TreeSet<DocTypeVo> parent) {
        this.parent = parent;
    }
    ...
    /**
     * @param tempate the tempate to set
     */
    ...
    public int compareTo(Object o1) {
        if (o1 instanceof DocTypeVo) {
            Long s1 = ((DocTypeVo) o1).getSortNum();
            Long s2 = this.getSortNum();
