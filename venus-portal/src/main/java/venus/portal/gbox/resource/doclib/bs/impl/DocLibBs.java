package venus.portal.gbox.resource.doclib.bs.impl;
...
public class DocLibBs extends BaseBusinessService implements IDocLibBs, IDocLibConstants {

    
    /**
     * dao 表示: 数据访问层的实例
     */
    private IDocLibDao dao = null;
     ...
     * @param dao
     */
    public void setDao(IDocLibDao dao) {
        this.dao = dao;
    }
    ...
    public OID insert(DocVo vo) {
        ...
        resVo.setType(ResourceTypeAPI.getDocType());
     ...
     */
    public int delete() {
        List<DocVo> list = getDao().queryByCondition("");
    ...
    public List<DocVo> queryByCondition(int no, int size, String queryCondition, String orderStr) {
        List<DocVo> lResult = getDao().queryByCondition(no, size, queryCondition, orderStr);
        return lResult;
    }
}
