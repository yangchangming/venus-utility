package venus.portal.gbox.resource.audiolib.util;

public interface IAudioLibConstants {
    
    //BS的规划化名称
    public final static String BS_KEY = "AudioLibBs";
    ...
    //Sql语句
    public final static String SQL_INSERT = "INSERT INTO GBOX_AUDIO ( ID, NAME, CODE, TAG, AUTHOR, IS_ORIGINAL,IS_EXTERNAL,IS_PROTECTED,ORIGIN,FILE_NAME,FILE_SIZE,FILE_FORMAT,FILE_STATUS,FILE_OBJECT,CREATOR_NAME,CREATE_DATE, DESCRIPTION) VALUES ( ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ? )";
    
    public final static String SQL_DELETE_BY_ID = "DELETE FROM GBOX_AUDIO WHERE ID=?";
    
    public final static String SQL_DELETE = "DELETE FROM GBOX_AUDIO ";

    public final static String SQL_QUERY_ALL = "SELECT ID id, NAME name, CODE code,TAG tag, AUTHOR author, IS_ORIGINAL isOriginal, IS_EXTERNAL isExternal,IS_PROTECTED isProtected,ORIGIN origin,CLICKS clicks,GOOD_COUNT good_count,BAD_COUNT bad_count,RECO_COUNT reco_count,FILE_NAME fileName,FILE_SIZE fileSize,FILE_FORMAT fileFormat,FILE_STATUS fileStatus, FILE_OBJECT fileObject,CREATOR_NAME creatorName,CREATE_DATE createDate, MODIFIER_NAME modifierName,MODIFY_DATE modifyDate, ENABLE_STATUS enableStatus, DESCRIPTION description FROM GBOX_AUDIO";
    
    public final static String SQL_FIND_BY_ID = SQL_QUERY_ALL + " WHERE ID=?";

    public final static String SQL_UPDATE_BY_ID = "UPDATE GBOX_AUDIO SET NAME=?, CODE=?,  MODIFIER_NAME=?,MODIFY_DATE=?, ENABLE_STATUS=?, DESCRIPTION=?  WHERE ID=?";
    
    public final static String SQL_UPDATETAG_BY_ID = "UPDATE GBOX_AUDIO SET TAG=?, MODIFIER_NAME=?,MODIFY_DATE=? WHERE ID=?";
    
    public final static String SQL_COUNT = "SELECT COUNT(ID) FROM GBOX_AUDIO";
    
    //表名
    public final static String TABLE_NAME = "GBOX_AUDIO";
