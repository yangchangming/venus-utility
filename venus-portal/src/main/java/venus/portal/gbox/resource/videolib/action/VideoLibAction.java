package venus.portal.gbox.resource.videolib.action;
...
public class VideoLibAction extends DefaultDispatchAction implements IVideoLibConstants {
     ...
     * @return BS对象
     */
    public IVideoLibBs getBs() {
        return (IVideoLibBs) Helper.getBean(BS_KEY);  //得到BS对象,受事务控制
    }    
    ...
    public IForward deleteMulti(DefaultForm formBean, IRequest request, IResponse response) throws Exception {
        ...
        String uploadPath = ResourceTypeAPI.getResourceTypeUploadPath(ResourceTypeAPI.getVideoType());
    ...
    public IForward detail(DefaultForm formBean, IRequest request, IResponse response) throws Exception {
        VideoVo vo = getBs().find(request.getParameter(REQUEST_ID));  //通过id获取vo
    ...
    public IForward upload(DefaultForm formBean, IRequest request, IResponse response) throws Exception {
        ...
        VideoVo vo = new VideoVo();
        String videoUploadPath = ResourceTypeAPI.getResourceTypeUploadPath(ResourceTypeAPI.getVideoType());
        String videoRealPath = videoUploadPath + FileTools.FILE_SEPARATOR + FileUpload.getDatePath();
        String uploadPath = FileUpload.getUploadPath(videoRealPath,isProtectedFile);
