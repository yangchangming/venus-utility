package venus.portal.gbox.resource.customizelib.bs.impl;
...
import venus.portal.gbox.resource.customizelib.dao.impl.CustomizeLibDao;
...
public class CustomizeLibBs extends BaseBusinessService implements ICustomizeLibBs, ICustomizeLibConstants {

    
    /**
     * dao 表示: 数据访问层的实例
     */
    private CustomizeLibDao dao = null;
     ...
     * @param dao
     */
    public void setDao(CustomizeLibDao dao) {
        this.dao = dao;
    }
    ...
    public OID insert(CustomizeVo vo) {
        IResourceBs resBs = (IResourceBs) Helper.getBean(IResourceConstants.BS_KEY);
     ...
     */
    public int delete() {
        List<CustomizeVo> list = getDao().queryByCondition("");
    ...
    public List<CustomizeVo> queryByCondition(int no, int size, String queryCondition, String orderStr) {
        List<CustomizeVo> lResult = getDao().queryByCondition(no, size, queryCondition, orderStr);
        return lResult;
    }
}
