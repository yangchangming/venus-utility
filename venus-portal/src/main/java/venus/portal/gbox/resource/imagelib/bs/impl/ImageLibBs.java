package venus.portal.gbox.resource.imagelib.bs.impl;
...
import venus.portal.gbox.resource.imagelib.dao.IImageLibDao;
...
public class ImageLibBs extends BaseBusinessService implements IImageLibBs, IImageLibConstants {
    
    /**
     * dao 表示: 数据访问层的实例
     */
    private IImageLibDao dao = null;
     ...
     * @param dao
     */
    public void setDao(IImageLibDao dao) {
        this.dao = dao;
    }
    ...
    public OID insert(ImageVo vo) {
		...
		resVo.setType(ResourceTypeAPI.getImageType());
     ...
     */
    public int delete() {
        List<ImageVo> list = getDao().queryByCondition("");
     ...
     * @return 查询到的VO对象
     */
    public ImageVo find(String id) {
		ImageVo vo = getDao().find(id);
		return vo;
    }
     ...
     * @return 成功更新的记录数
     */
    public int updateTag(ImageVo vo) {
        IResourceBs resBs = (IResourceBs) Helper.getBean(IResourceConstants.BS_KEY);
        ResourceVo resVo = new ResourceVo();
        resVo.setTag(vo.getTag());
        resVo.setModifierName(vo.getModifierName());
     ...
     * @return 查询到的VO列表
     */
    public List<ImageVo> queryByCondition(int no, int size, String queryCondition, String orderStr) {
		List<ImageVo> lResult = getDao().queryByCondition(no, size, queryCondition, orderStr);
		return lResult;
    }
