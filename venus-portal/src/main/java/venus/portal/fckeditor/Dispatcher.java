 ...
 * == END LICENSE ==
 */
package venus.portal.fckeditor;

import net.fckeditor.connector.Connector;
import net.fckeditor.connector.exception.*;
...
//import venus.portal.dependence.gbox.common.upload.FckForGboxFileUpload;
...
public class Dispatcher {
    ...
    private final int MAX_FILE_ERROR = 204;//上传文件大于最大配置时的返回数值，（新添加）
     ...
     * initialization}.
     *
     * @param servletContext reference to the {@link ServletContext} in which the caller is
     *                       running
     * @throws Exception if the dispatcher initialization fails due to some reason
     */
     ...
     * a helper method.
     *
     * @param command        the current command, should be only GetFolders or
     *                       GetFoldersAndFiles
     * @param type           current resource type
     ...
     * @throws ReadException                 if the file attributes and/or folder names could not be read
     *                                       due to some reason
     */
     ...
     */
    UploadResponse doPost(final HttpServletRequest request, long maxSize, boolean managedByGbox) {
        ...
        if (!RequestCycleHandler.isFileUploadEnabled(request))
            ...
            try {
                ...
                if (uplFile.getSize() > maxSize) {
                    return new UploadResponse(new Object[]{MAX_FILE_ERROR});
                ...
                if (type.isDeniedExtension(FilenameUtils.getExtension(fileName)))
                    ...
                    //如果web.xml中设置了fckeditor上传的文件要Gbox管理起来将FckEditor中上传的文件在gbox中也管理起来。在gbox方面采用数据导入的方式
                    //gbox功能暂时屏蔽掉，因为gbox上传资源时需要flex服务端支持，没有必要这种上传文件功能更需要一个flex服务端，太重！fckedit上传文件默认放在根目录下即可
//                    if(managedByGbox){
//                       FckForGboxFileUpload.fckResourceToGbox(request,uplFile,fileUrl,type);
//                    }
                }
                uplFile.delete();
            } catch (InvalidCurrentFolderException e) {
