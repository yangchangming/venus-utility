...
public class ConnectorServlet extends HttpServlet {
	...
	private long maxSize =10*1024*1024;//默认文件最大10M
	private boolean managedByGbox = false ; //fckeditor的上传文件是否被资源管理管理起来的参数
	...
	public void init() throws ServletException {
		try {
			...
			String tempSize= getInitParameter("maxSize");
			if(tempSize!=null &&tempSize!= ""){
			    maxSize =Long.parseLong(tempSize)*1024;
			}
		 String  tempManaged = getInitParameter("managedByGbox");
		if(tempManaged!=null&&"true".equals(tempManaged)){
		    managedByGbox= true;
		}
		} catch (Exception e) {
	...
	protected void doPost(final HttpServletRequest request, final HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			ThreadLocalData.beginRequest(request);
			uploadResponse = dispatcher.doPost(request,maxSize,managedByGbox);
		} catch (Exception e) {
			throw new ServletException(e);
		} finally {
			/*
