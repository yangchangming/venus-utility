...
//            return MessageAgent.sendErrorMessage(request, DEFAULT_MSG_ERROR_STR, MessageStyle.ALERT_AND_BACK);
            return IConstants.MESSAGE_AGENT_ERROR;
        }
        ...
        auPartyBs.addPartyAndRelation(vo, parentRelId, relType);
...
//        return request.findForward(FORWARD_LIST_PAGE_KEY);
    ...
    public String delete(HttpServletRequest request, HttpServletResponse response) throws Exception {
        auPartyBs.delete(request.getParameter("id"));//删除团体和团体关系
...
//        return request.findForward(FORWARD_LIST_PAGE_KEY);
    ...
    public String find(HttpServletRequest request, HttpServletResponse response) throws Exception {
        PartyVo bean = (PartyVo)auPartyBs.find(request.getParameter("id")); //通过id获取vo
        request.setAttribute(IConstants.REQUEST_BEAN_VALUE, bean); //把vo放入request
//        return request.findForward(FORWARD_UPDATE_KEY);
        return IConstants.FORWARD_UPDATE_KEY;
    }
...
//        return request.findForward(FORWARD_LIST_PAGE_KEY);
    ...
    public String detail(HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        PartyVo bean = (PartyVo)auPartyBs.find(id); //通过id获取vo
...
//        return request.findForward(FORWARD_DETAIL_KEY);
        return IConstants.FORWARD_DETAIL_KEY;
    }
...
//        IAuPartyBs bs = getBs();
        ...
        if (pageVo != null) {
            ...
            int recordCount = auPartyBs.getRecordCountPerson(queryCondition);
            pageVo = Helper.createPageVo(request, recordCount);
        }
        String orderStr = Helper.findOrderStr(request);
        
    
        List list = auPartyBs.simpleQueryPerson(pageVo.getCurrentPage(), pageVo.getPageSize(), orderStr, queryCondition);//查询用户列表
