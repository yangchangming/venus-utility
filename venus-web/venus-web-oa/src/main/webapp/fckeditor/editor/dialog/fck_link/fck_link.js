...
if ( !FCKConfig.LinkDlgHideAdvanced )
	...
	//新加资源管理链接tab
if( !FCKConfig.linkDlgHideGbox )
   dialog.AddTab("GboxLink" ,FCKLang.DlgGboxLinkTag);
...
function OnDialogTabChange( tabCode )
{
	...
	//对新加资源链接 tab进行显示
    ShowE('gboxDivInfo'  , ( tabCode == 'GboxLink' ) ) ;
...
function Ok()
{
	...
	//处理 gbox资源链接
	var gboxUrl = GetE('txtGboxLnkUrl').value;
	var gboxResourceName =GetE('gboxSourceName').value;
	...
	for ( var i = 0 ; i < aLinks.length ; i++ )
	{
        ...
        //如果选择的是资源库文件，则链接名字修改为资源库文件名
		 if(gboxResourceName==null||gboxResourceName==""){
          ...
          oLink.innerHTML = tempInnerHtml;
		 	 ...
		 	 oLink.innerHTML  =gboxResourceName;
		 }
		
          //以上3行是 去掉上传后表单中文件名前的路径字段
		// Target
		if( GetE('cmbTarget').value != 'popup' )
...
//打开资源库资源按钮事件处理js
function BrowseGboxServer()
{
	OpenFileBrowser("/"+FCKConfig.AppName + "/ResourceAction.do?cmd=queryResource",FCKConfig.ImageBrowserWindowWidth,FCKConfig.ImageBrowserWindowHeight );
}
...
function OnUploadCompleted( errorNumber, fileUrl, fileName, customMsg )
{
	...
	switch ( errorNumber )
	{
        ...
        case 101 :	// Custom warning
            ...
            break ;
        case 201 :
            alert( ((FCKLang.AlertMessageFileRenamed!=null)?FCKLang.AlertMessageFileRenamed:"A file with the same name is already available. The uploaded file has been renamed to")+'"' + fileName + '"' ) ;
            break ;
        case 202 :
            alert( (FCKLang.AlertMessageInvalidFileType!=null)?FCKLang.AlertMessageInvalidFileType:"Invalid File Type" ) ;
            return ;
        case 203 :
            alert( (FCKLang.AlertMessageNoPermissionsToUpload!=null)?FCKLang.AlertMessageNoPermissionsToUpload:"Security error. You probably don't have enough permissions to upload. Please check your server." ) ;
            return ;
        case 204 :
            alert( (FCKLang.AlertMessageFileSizeLimitIsExceeded!=null)?FCKLang.AlertMessageFileSizeLimitIsExceeded:"File Size Limit Is Exceeded" ) ;
            return ;
        case 500 :
            alert( (FCKLang.AlertMessageConnectorDisabled!=null)?FCKLang.AlertMessageConnectorDisabled:"The connector is disabled" ) ;
            break ;
        default :
            alert( ((FCKLang.AlertMessageErrorOnFileUploadNumber!=null)?FCKLang.AlertMessageErrorOnFileUploadNumber:"Error on file upload. Error number: ") + errorNumber ) ;
            return ;
	}
