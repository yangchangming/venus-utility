...
<% 
    List lResult = null;
    if(request.getAttribute(ITagConstants.REQUEST_BEANS) != null) { 
        lResult = (List)request.getAttribute(ITagConstants.REQUEST_BEANS);
    }
...
<script language="javascript">
    ...
    function toMofidy_onClick() {  //从多选框到修改页面
        ...
        if(ids.length > 1) {  //如果ids有2条以上的纪录
          ...
          jQuery("form").attr("action","<venus:base/>/TagAction.do?id=" + ids);
    ...
    function deleteMulti_onClick(){ 
        ...
        if(confirm('<fmt:message key="whether_to_delete_the_data_completely" bundle="${applicationResources}"/>')) { 
            jQuery("form").attr("action","<venus:base/>/TagAction.do?ids=" + ids);
    ...
    function simpleQuery_onClick(){  //简单的模糊查询
            jQuery("form").attr("action","<venus:base/>/TagAction.do?backFlag=true");
            jQuery("input[name='cmd']").val("simpleQuery");
            jQuery("form").submit();
    }
    
    function toAdd_onClick() {  //到增加记录页面
         jQuery("form").attr("action","<venus:base/>/jsp/ewp/tag/insertTag.jsp");
          jQuery("form").submit();
    }
    ...
    jQuery(function(){
        jQuery("td table tr").bind("dblclick",function(){
            ...
            if(id != null && id != "undefined"){
                form.action="<venus:base/>/TagAction.do?id=" + id;
...
<div style="width:100%;">
<script language="javascript">
     writeTableTop('<fmt:message key="udp.ewp.tag_manage" />','<venus:base/>/themes/<venus:theme/>/');
</script>
<form name="form" method="post" action="<venus:base/>/TagAction.do?backFlag=true">
...
<table class="table_div_content" border="0">
        <tr>
            <td align="center">
                <fmt:message key="udp.ewp.tag_name"/>
                <input type="text" class="text_field" name="name" inputName='<fmt:message key="udp.ewp.tag_name"/>' maxLength="50" validate="isSearch"/>
...
<table class="table_div_content2">
    <tr>
        ...
        <layout:collection name="beans" id="rmBean" styleClass="listCss" width="100%" indexId="orderNumber" align="center" sortAction="0">
            ...
            <layout:collectionItem width="8%" title='<%=LocaleHolder.getMessage("udp.ewp.tag_name") %>' property="name" sortable="true"/>
