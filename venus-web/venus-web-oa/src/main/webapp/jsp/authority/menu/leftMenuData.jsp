    ...
    function menu_onclick(submenu ,cellbar,i) {
        ...
        if( old_menu != submenu ) {
            if( old_menu !='' ) { //旧菜单缩回
                ...
                jQuery("#"+old_cell).attr("src", "<venus:base/>/themes/<venus:theme/>/images/au/leftmenu_close.gif");
            }
            //新菜单伸出
            jQuery(".tree_Menu:eq("+i+")").css('display','block');//td修改css
            jQuery("#"+submenu).slideDown(200);
            jQuery("#"+cellbar).attr("src",  "<venus:base/>/themes/<venus:theme/>/images/au/leftmenu_open.gif" );
            old_menu = submenu;
            old_cell = cellbar;
        } else {
            //新菜单收回
            jQuery("#"+submenu).slideUp(200);
            jQuery("#"+cellbar).attr("src",  "<venus:base/>/themes/<venus:theme/>/images/au/leftmenu_close.gif" );
            old_menu = '';
            old_cell = '';
             tr_num='';
        }
        ...
        for(var i=0;i<tds.length;i++) {
            with(tds[i]) {
                onmouseover=function() {
                    with(this) {
                        style.background='#ecebeb'; //这是鼠标移上去时的背景颜色
                ...
                onmouseout=function() {
                    with(this) {
                        style.background='#e0e0e0'; //这是鼠标离开时的背景颜色
...
%>
    <table id="tree<%=i %>" align="center" border="0" cellpadding="0" cellspacing="0" class="tree_table">
        <tr>
            ...
            &nbsp;&nbsp;<img id="bar<%=i%>" src="<venus:base/>/themes/<venus:theme/>/images/au/leftmenu_close.gif" width="16" height="16" border="0" align="absmiddle" />&nbsp;<%=svo.getName()%>
            </td>
        </tr>
        <tr>
            <td  class="tree_Menu" >
