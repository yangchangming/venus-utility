...
<script language="javascript">
	function insert_onClick(){  //插入单条数据
     ...
     var roleName=jQuery("#name").val();
    if(roleName==null||roleName==""){
          alert("<fmt:message key='venus.authority.role_name_not_null' bundle='${applicationAuResources}' />");
         jQuery("#name").focus();
       return false;
     }
      var refPath="<%=request.getContextPath()%>/relation/checkRoleName";
      jQuery.get(refPath, {Action:"get",parentRelId:parentRelationId,role_name:roleName,operation:"add",async:false}, function (data){
             if(data=="1"){//1表示通过
                    form.action="<%=request.getContextPath()%>/auRole/insert";
     ...
     if(roleName==null||roleName==""){
      ...
      var refPath="<%=request.getContextPath()%>/relation/checkRoleName";
      jQuery.get(refPath, {Action:"get",parentRelId:parentRelationId,role_name:roleName,operation:"update",async:false}, function (data){
             if(data=="1"){//1表示通过          
		        form.action="<%=request.getContextPath()%>/auRole/update";
		        var parentRelId = form.parentRelId.value;
		        if(parentRelId!="")
		            form.target="_parent";
		        form.submit();
...
<table class="table_noFrame">
	<tr>
		<td>
			<input name="button_save" class="button_ellipse" type="button" value="<fmt:message key='venus.authority.Save' bundle='${applicationAuResources}' />" onClick="javascript:<%=isModify?"update_onClick()":"insert_onClick()"%>">
	...
	<tr>
		<td> 
			<table class="table_div_content_inner">
				<tr>
					<td width="30%" align="right"><span class="style_required_red">* </span><fmt:message key='venus.authority.Role_name' bundle='${applicationAuResources}' /></td>
					<td width="70%" align="left">
						<input type="text" class="text_field" id="name"  name="name" inputName="<fmt:message key='venus.authority.Role_name' bundle='${applicationAuResources}' />" validate="notNull;isSearch" value="" maxLength="300" />
					</td>
				</tr>
				<tr>
