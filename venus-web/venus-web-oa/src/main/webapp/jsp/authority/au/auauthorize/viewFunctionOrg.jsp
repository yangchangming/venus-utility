...
%>
				...
				var node<%=vo.getTotal_code()%> = new WebFXTreeItem("<%=vo.getName()%><%=resSet.contains(vo.getId())?"[<fmt:message key='venus.authority.Open' bundle='${applicationAuResources}' />]":""%>","<%=vo.getName()%>","<%=auVo!=null?auVo.getId():null%>","<%=vo.getTotal_code()%>",selTxtArray,selValArray,new Array(0),new Array('<%=selfType%>','<%=selfAccs%>'),"<%=isDisable%>",false);
<%
			}else {
