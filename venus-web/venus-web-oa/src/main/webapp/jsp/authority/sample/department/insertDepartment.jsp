...
{ 
	...
	function insert_onClick(){  //插入单条数据
        ...
        var deptName=jQuery("#dept_name").val();
        var partyId=jQuery("#owner_party_id").val();
        var refPath = "<%=request.getContextPath()%>/relation/checkDeptName";
        jQuery.get(refPath, {Action: "get", parentRelId: parentRelationId, dept_name: deptName, operation: "add", partyId:partyId, async: false}, function (data) {
            if (data == "1") {//1表示通过
                form.action = "<%=request.getContextPath()%>/department/insert";
     ...
     var partyId=jQuery("input[name='id']").val();
      var refPath="<%=request.getContextPath()%>/relation/checkDeptName";
      jQuery.get(refPath, {Action:"get",parentRelId:parentRelationId,dept_name:deptName,operation:"update",partyId:partyId, async:false}, function (data){
             if(data=="1"){//1表示通过
                    form.action="<%=request.getContextPath()%>/department/update";
			        var parentRelId = form.parentRelId.value;
			        if(parentRelId!="")
			            form.target="_parent";
			        form.submit();
	...
	<tr>
		<td> 
			<table class="table_div_content_inner">
				<tr>
					<td width="30%" align="right"><span class="style_required_red">* </span><fmt:message key='venus.authority.Department_name' bundle='${applicationAuResources}' /></td>
					<td width="70%" align="left">
						<input type="text" class="text_field" id="dept_name" name="dept_name" inputName="<fmt:message key='venus.authority.Department_name' bundle='${applicationAuResources}' />" validate="notNull;isSearch;" value="" maxLength="100" />
					</td>
				</tr>
				<%
				...
				%>
				<tr>
					<td align="right"><span class="style_required_red">* </span><fmt:message key='venus.authority.Owned_companies_or_departments_' bundle='${applicationAuResources}' /></td>
					<td align="left">
						<input type="text" class="text_field_reference_readonly" id="owner_party_name" name="owner_party_name" inputName="<fmt:message key='venus.authority.Owned_companies_or_departments_' bundle='${applicationAuResources}' />" maxlength="50" validate="notNull;" 
						...
						onClick="javascript: try { getTree(new Array('owner_party_id', 'owner_party_name'),'<venus:base/>','radio','','1','id','2','1'); }catch(e){alert('<fmt:message key='venus.authority.Insufficient_privileges' bundle='${applicationAuResources}' />')}"/>
					</td>
				</tr>
				<%
				}
				...
				<tr>
					...
					<td align="left">
						<input type="text" class="text_field" name="dept_flag" inputName="<fmt:message key='venus.authority.Department_logo' bundle='${applicationAuResources}' />" validate="isSearch;" value="" maxLength="18" />
					</td>
				</tr>
				<tr>
					<td align="right"><fmt:message key='venus.authority.Department_Type' bundle='${applicationAuResources}' /></td>
					<td align="left">
						<input type="text" class="text_field" name="dept_type" inputName="<fmt:message key='venus.authority.Department_Type' bundle='${applicationAuResources}' />" validate="isSearch;" value="" maxLength="18" />
					</td>
				</tr>
				<tr>
					<td align="right"><fmt:message key='venus.authority.Sectoral_level' bundle='${applicationAuResources}' /></td>
					<td align="left">
						<input type="text" class="text_field" name="dept_level" inputName="<fmt:message key='venus.authority.Sectoral_level' bundle='${applicationAuResources}' />" validate="isSearch;" value="" maxLength="1" />
					</td>
				</tr>
				<tr>
					<td align="right"><fmt:message key='venus.authority.Responsible_person' bundle='${applicationAuResources}' /></td>
					<td align="left">
						<input type="text" class="text_field" name="dept_leader" inputName="<fmt:message key='venus.authority.Person_in_charge' bundle='${applicationAuResources}' />" validate="isSearch;" value="" maxLength="18" />
					</td>
				</tr>
				<!--tr>
