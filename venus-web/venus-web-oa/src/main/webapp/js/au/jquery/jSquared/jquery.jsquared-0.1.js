...
(function($) {
	...
	$.fn.jSquared = function(settings) {
		...
		function init(k, o) {
			...
			if ($t.find('tr:first th').length==0)
				...
				$t.find('tr:first').find('th:not(:first)').each( function() {
					var $h = $(this);
					var text = $h.html();
					//don't operate if the class attr is 'fixed'.
					if($h.attr('class').indexOf('fixed')<0){
					   $newH = $( renderColumnHeader(text,o,text) );
                        $newH.insertBefore( $h ); 
                        $h.remove();
			...
			$t.find('tr').find('th:last').after('<th class="jSquaredAddCol"><select><option value="jp">日文显示内容</option><option value="ru">俄文显示内容</option></select><a href="javascript:void(0)">'+o.addColumnText+'</a></th>');
		...
		function renderColumnHeader(text, o,v) {
			return '<th class="jSquaredColumnText" id="'+v+'"><span class="text">'+text+'</span><a class="jSquaredColumnRemove" href="javascript:void(0)">'+o.removeRowText+'</a></th>';
		}
		...
		function onAddCol(cell, o) {
			var $t = cell.parents('table.jSquaredTable:first');
			var $addTh = cell;
			var $input = $addTh.find('select');
			var $select = $addTh.find('select').find("option:selected");
			var text = $select.text();
			if (text=='')
				return;			
			var thIndex = $addTh.parents('tr:first').find('th').index($addTh);
			// render col header
			var $newTh = $(renderColumnHeader(text,o,$input.val()));
		    ...
		    o.onColumnAdded($newTh, $input.val(), $t.find('tr:not(:last)').find('td:eq('+thIndex+')') );
		}
