        ...
        try {
			if(initConfig(xssPath)){
                ...
                for(Map matchMap : checkUrlMatcherList){
                    logger.info("特殊URL过滤匹配规则" + matchMap);
                }

			}else{
                logger.error("初始化XSS配置失败!");
	...
	public static boolean initConfig(String path) throws DocumentException {
        ...
        if(checkUrlEles!=null){
            ...
            while(checkUrls.hasNext()){
                ...
                Map<String,Object> checkUrlMatch = new HashMap();
                ...
                if(!"".equals(tempCheckUrlKey)){
                    ...
                    logger.error("安全过滤配置文件加载失败:特殊url正则表达式异常,url匹配为 " + tempCheckUrlKey);
                    return false;
                }
            }

        }else{
            logger.error("安全过滤配置文件中没有 "+ XSSSecurityConstants.CHECK_URL_LIST+" 属性");
            return false;
        }
		...
		if(regexEle != null){
			...
			logger.error("安全过滤配置文件中没有 "+ XSSSecurityConstants.REGEX_LIST+" 属性");
			return false;
		}
