...
//                    Object o = bindNonPrimitiveSet(httpParamMap, methodParamName, parameterType);
                ...
                //or {"name":"zhangsan","age":22}
            }else if (parameter.isAnnotationPresent(RequestBody.class)){
                String _bodyContent = Mvcs.requestBody2Str(context.getRequest());
                if (_bodyContent!=null && !"".equals(_bodyContent)){
                ...
                if (MIMEType.MIME_TYPE_JSON.equals(context.getRequest().getContentType())){
                    ...
                    //name="xxx"&age=22&sex=2&birthday="2018-11-19"
                    if (bodyContent!=null && !"".equals(bodyContent) && bodyContent.indexOf("=")>-1){
                        ...
                        //or "{"name":"zhangsan","age":22}"
                    }else if (bodyContent!=null && !"".equals(bodyContent) && bodyContent.indexOf(":")>-1){

                        JSONObject jsonObject = JSONObject.fromObject(bodyContent);

                        for (Object o : jsonObject.keySet()) {
                            String key = (String)o;
                            String value = jsonObject.get(key)==null?"":String.valueOf(jsonObject.get(key));
                            httpBodyMap.put(key,  value);
                        }
                    }
                    ...
                    if (Clazz.isPrimitive(parameterType)){
                        String paramVaule = "";
                        //编译时必须加上编译参数(-parameters)，否则无法获取方法名称
                        if (httpBodyMap!=null && httpBodyMap.containsKey(parameter.getName())){
                            paramVaule = httpBodyMap.get(parameter.getName());
                        }
                        o = Castor.stringToClzInstance(paramVaule, parameterType);
                    }else {
